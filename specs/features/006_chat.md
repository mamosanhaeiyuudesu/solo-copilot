...
Claude とリアルタイムでストリーミング対話できるチャット機能。会話履歴を D1 に永続化し、チャットのやり取りから自動的に中間記憶を抽出する。システムプロンプトはユーザーが設定できる（living_profile の注入は廃止）。

## 背景・動機

Phase 2-b：「自分のデータを知った上でのAI対話」を実現する基盤。会話履歴は永続化され、蓄積した中間記憶が記憶タイムラインとして可視化される（チャットへの直接注入は廃止）。
...
| POST | `/api/agent/chat` | 要 | Claude ストリーミング応答 |
...
デフォルトプロンプトは傾聴・受け止め・寄り添いを重視するカウンセラーペルソナ。living_profile の注入は廃止（記憶はタイムラインで別途閲覧する）。
