...
export default defineEventHandler(async (event) => {
  ...
  if (query.sourceType && ['raw_external_data', 'task'].includes(query.sourceType as string)) {
    conditions.push(eq(intermediateRecords.sourceType, query.sourceType as 'raw_external_data' | 'task'))
  }
  if (query.dateFrom) {
